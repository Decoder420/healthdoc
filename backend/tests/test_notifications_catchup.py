...
async def test_catch_up_returns_all_history_when_no_last_event_id(db):
    ...
    row1 = NotificationHistory(
        ...
        department_id=department_id, created_at=now - timedelta(minutes=10),
    )
    row2 = NotificationHistory(
        id=uuid.uuid4(), event_type="critical_value_alert", payload={"b": 2},
        department_id=department_id, created_at=now - timedelta(minutes=5),
    )
    ...
    result = await _catch_up_department_events(db, department_id, None)
    assert len(result) == 2
    assert result[0].id == row1.id  # ascending by created_at
...
async def test_catch_up_only_returns_events_after_last_event_id(db):
    ...
    old_row = NotificationHistory(
        ...
        department_id=department_id, created_at=now - timedelta(minutes=20),
    )
    new_row = NotificationHistory(
        id=uuid.uuid4(), event_type="lab_report_ready", payload={},
        department_id=department_id, created_at=now - timedelta(minutes=1),
    )
...
async def test_catch_up_ignores_malformed_last_event_id(db):
    """A garbled Last-Event-ID (e.g. from a very old client) shouldn't
    crash the stream -- it should just fall back to returning everything,
    same as no header at all."""
    department_id = uuid.uuid4()
    row = NotificationHistory(
        id=uuid.uuid4(), event_type="low_stock_alert", payload={},
        department_id=department_id, created_at=datetime.now(timezone.utc),
    )
    db.add(row)
    await db.flush()
...
async def test_catch_up_scopes_by_department(db):
    ...
    row_a = NotificationHistory(
        id=uuid.uuid4(), event_type="low_stock_alert", payload={}, department_id=dept_a, created_at=now,
    )
    row_b = NotificationHistory(
        id=uuid.uuid4(), event_type="low_stock_alert", payload={}, department_id=dept_b, created_at=now,
    )
