...
class OrderStatus(BaseEnum):
    # NOTE: schema.md names this enum but doesn't spell out every value —
    # this is my best-guess starter set. Confirm the real list with
    # whoever owns enums.py before merging; lab/radiology/pharmacy each
    # track their own finer-grained statuses downstream of this one.
    PLACED = "placed"
    IN_PROGRESS = "in_progress"
    COMPLETED = "completed"
    CANCELLED = "cancelled"
...
class PrescriptionItemStatus(BaseEnum):
    # Same note as OrderStatus — reasonable starting values, confirm before merge.
    PRESCRIBED = "prescribed"
    DISPENSED = "dispensed"
    PARTIALLY_DISPENSED = "partially_dispensed"
    ON_HOLD = "on_hold"
...
class AdmissionStatus(BaseEnum):
    # These come straight from architecture.md §20.2 — this one IS
    # authoritative, not a guess.
    ADMISSION_REQUESTED = "admission_requested"
    BED_PENDING = "bed_pending"
    ADMITTED = "admitted"
    WARD_TRANSFER_PENDING = "ward_transfer_pending"
    TRANSFERRED = "transferred"
    DISCHARGE_PLANNED = "discharge_planned"
    DISCHARGE_SUMMARY_PENDING = "discharge_summary_pending"
    BILLING_CLEARANCE_PENDING = "billing_clearance_pending"
    DISCHARGED = "discharged"
    REFERRED = "referred"
    LAMA = "lama"
    DEATH = "death"
