import { FACILITY_ID, FACILITY_CODE } from "@/features/audit-viewer/constants";
import type {
  AuditIntegrityCheck,
  AuditLog,
  AuditLogArchive,
  DataAccessLog,
  FileAccessLog,
} from "@/features/audit-viewer/types";

/** Deterministic fake sha256-looking hex (64 chars). */
function h(seed: string): string {
  let n = 0;
  for (let i = 0; i < seed.length; i++) n = (n * 31 + seed.charCodeAt(i)) >>> 0;
  const hex = (n.toString(16) + "a1b2c3d4e5f67890".repeat(4)).slice(0, 64);
  return hex.padEnd(64, "0");
}

const GENESIS = "0".repeat(64);

const USR_REC = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa10";
const USR_DOC = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa21";
const USR_LAB = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa31";
const USR_ADM = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1";
const USR_BILL = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa40";
const USR_RAD = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaa51";

const PAT1 = "30000000-0000-4000-8000-000000000001";
const PAT2 = "30000000-0000-4000-8000-000000000002";
const PAT3 = "30000000-0000-4000-8000-000000000003";
const VIS1 = "20000000-0000-4000-8000-000000000001";
const VIS2 = "20000000-0000-4000-8000-000000000008";
const INV1 = "10000000-0000-4000-8000-000000000001";

type AuditSeedRow = {
  id: string;
  created_at: string;
  facility_id: string;
  user_id: string | null;
  role: string | null;
  department_id: string | null;
  action: string;
  resource_type: string;
  resource_id: string | null;
  patient_id: string | null;
  visit_id: string | null;
  old_value: Record<string, unknown> | null;
  new_value: Record<string, unknown> | null;
  reason: string | null;
  ip_address: string | null;
  device_id: string | null;
  signature: string;
  signer_key_id: string;
  user_display?: string;
  patient_display?: string;
};

const SEED_AUDIT: AuditLog[] = (() => {
  const rows: AuditSeedRow[] = [
    {
      id: "70000000-0000-4000-8000-000000000001",
      created_at: "2026-07-01T02:42:00.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_REC,
      role: "receptionist",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000001",
      action: "create",
      resource_type: "patients",
      resource_id: PAT1,
      patient_id: PAT1,
      visit_id: null,
      old_value: null,
      new_value: { uhid: "UHID-1001", full_name: "Anita Sharma" },
      reason: "OPD registration",
      ip_address: "10.0.1.21",
      device_id: "desk-reg-01",
      signature: "ed25519:sig-aud-001",
      signer_key_id: "audit-key-2026-q2",
      user_display: "R. Mehta",
      patient_display: "Anita Sharma",
    },
    {
      id: "70000000-0000-4000-8000-000000000002",
      created_at: "2026-07-01T02:45:22.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_REC,
      role: "receptionist",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000001",
      action: "create",
      resource_type: "visits",
      resource_id: VIS1,
      patient_id: PAT1,
      visit_id: VIS1,
      old_value: null,
      new_value: { visit_type: "opd", department: "General Medicine" },
      reason: null,
      ip_address: "10.0.1.21",
      device_id: "desk-reg-01",
      signature: "ed25519:sig-aud-002",
      signer_key_id: "audit-key-2026-q2",
      user_display: "R. Mehta",
      patient_display: "Anita Sharma",
    },
    {
      id: "70000000-0000-4000-8000-000000000003",
      created_at: "2026-07-02T04:34:11.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_DOC,
      role: "doctor",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000002",
      action: "update",
      resource_type: "visits",
      resource_id: VIS1,
      patient_id: PAT1,
      visit_id: VIS1,
      old_value: { status: "waiting" },
      new_value: { status: "in_consultation" },
      reason: null,
      ip_address: "10.0.2.44",
      device_id: "opd-tablet-03",
      signature: "ed25519:sig-aud-003",
      signer_key_id: "audit-key-2026-q2",
      user_display: "Dr. K. Iyer",
      patient_display: "Anita Sharma",
    },
    {
      id: "70000000-0000-4000-8000-000000000004",
      created_at: "2026-07-03T05:50:00.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_LAB,
      role: "lab_tech",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000003",
      action: "create",
      resource_type: "lab_orders",
      resource_id: "40000000-0000-4000-8000-000000004401",
      patient_id: PAT2,
      visit_id: VIS2,
      old_value: null,
      new_value: { tests: ["CBC", "RBS"] },
      reason: null,
      ip_address: "10.0.3.12",
      device_id: "lab-ws-02",
      signature: "ed25519:sig-aud-004",
      signer_key_id: "audit-key-2026-q2",
      user_display: "S. Nair",
      patient_display: "Rahul Verma",
    },
    {
      id: "70000000-0000-4000-8000-000000000005",
      created_at: "2026-07-05T03:31:45.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_ADM,
      role: "admin",
      department_id: null,
      action: "merge",
      resource_type: "patients",
      resource_id: PAT3,
      patient_id: PAT3,
      visit_id: null,
      old_value: { duplicate_of: "30000000-0000-4000-8000-000000000099" },
      new_value: { survivor: PAT3, merged_from: "30000000-0000-4000-8000-000000000099" },
      reason: "Duplicate UHID cleanup",
      ip_address: "10.0.0.5",
      device_id: "admin-laptop",
      signature: "ed25519:sig-aud-005",
      signer_key_id: "audit-key-2026-q2",
      user_display: "A. Choudhary",
      patient_display: "Merged patient",
    },
    {
      id: "70000000-0000-4000-8000-000000000006",
      created_at: "2026-07-08T09:03:10.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_BILL,
      role: "receptionist",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000004",
      action: "update",
      resource_type: "invoices",
      resource_id: INV1,
      patient_id: PAT1,
      visit_id: VIS1,
      old_value: { status: "draft", scheme_code: null },
      new_value: { status: "issued", scheme_code: "PMJAY" },
      reason: "Issue invoice after scheme confirm",
      ip_address: "10.0.4.8",
      device_id: "bill-desk-01",
      signature: "ed25519:sig-aud-006",
      signer_key_id: "audit-key-2026-q2",
      user_display: "P. Joshi",
      patient_display: "Anita Sharma",
    },
    {
      id: "70000000-0000-4000-8000-000000000007",
      created_at: "2026-07-10T02:25:00.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_DOC,
      role: "doctor",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000002",
      action: "login",
      resource_type: "users",
      resource_id: USR_DOC,
      patient_id: null,
      visit_id: null,
      old_value: null,
      new_value: { session: "started" },
      reason: null,
      ip_address: "10.0.2.44",
      device_id: "opd-tablet-03",
      signature: "ed25519:sig-aud-007",
      signer_key_id: "audit-key-2026-q2",
      user_display: "Dr. K. Iyer",
    },
    {
      id: "70000000-0000-4000-8000-000000000008",
      created_at: "2026-07-12T10:40:30.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_ADM,
      role: "admin",
      department_id: null,
      action: "export",
      resource_type: "patients",
      resource_id: null,
      patient_id: null,
      visit_id: null,
      old_value: null,
      new_value: { format: "csv", rows: 120 },
      reason: "Monthly quality report",
      ip_address: "10.0.0.5",
      device_id: "admin-laptop",
      signature: "ed25519:sig-aud-008",
      signer_key_id: "audit-key-2026-q2",
      user_display: "A. Choudhary",
    },
    {
      id: "70000000-0000-4000-8000-000000000009",
      created_at: "2026-07-15T07:10:00.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_RAD,
      role: "radiology_tech",
      department_id: "cccccccc-cccc-4ccc-8ccc-000000000005",
      action: "create",
      resource_type: "radiology_orders",
      resource_id: "40000000-0000-4000-8000-000000007701",
      patient_id: PAT2,
      visit_id: VIS2,
      old_value: null,
      new_value: { modality: "XRAY", body_part: "Chest PA" },
      reason: null,
      ip_address: "10.0.5.19",
      device_id: "rad-ws-01",
      signature: "ed25519:sig-aud-009",
      signer_key_id: "audit-key-2026-q2",
      user_display: "M. Rao",
      patient_display: "Rahul Verma",
    },
    {
      id: "70000000-0000-4000-8000-000000000010",
      created_at: "2026-07-17T12:32:15.000Z",
      facility_id: FACILITY_ID,
      user_id: USR_ADM,
      role: "admin",
      department_id: null,
      action: "delete_attempt",
      resource_type: "audit_logs",
      resource_id: "70000000-0000-4000-8000-000000000001",
      patient_id: null,
      visit_id: null,
      old_value: null,
      new_value: { blocked: true },
      reason: "Trigger blocked UPDATE/DELETE",
      ip_address: "10.0.0.5",
      device_id: "admin-laptop",
      signature: "ed25519:sig-aud-010",
      signer_key_id: "audit-key-2026-q2",
      user_display: "A. Choudhary",
    },
  ];

  let prev = GENESIS;
  return rows.map((row, i) => {
    const payload = `${prev}|${row.id}|${row.created_at}|${row.action}|${row.resource_type}`;
    const entry_hash = h(payload + String(i));
    prev = entry_hash;
    // Slim §4.4 AuditLogOut — extras above are seed-only / integrity internals
    const slim: AuditLog = {
      id: row.id,
      user_id: row.user_id,
      role: row.role,
      action: row.action,
      resource_type: row.resource_type,
      resource_id: row.resource_id,
      patient_id: row.patient_id,
      old_value: row.old_value,
      new_value: row.new_value,
      ip_address: row.ip_address,
      created_at: row.created_at,
      entry_hash,
      user_display: row.user_display,
      patient_display: row.patient_display,
    };
    return slim;
  });
})();

const SEED_DATA_ACCESS: DataAccessLog[] = [
  {
    id: "80000000-0000-4000-8000-000000000001",
    user_id: USR_DOC,
    role: "doctor",
    resource_type: "patients",
    resource_id: PAT1,
    patient_id: PAT1,
    purpose_code: "treatment",
    access_channel: "ui",
    emergency_access: false,
    consent_verified: true,
    accessed_at: "2026-07-02T04:30:00.000Z",
    user_display: "Dr. K. Iyer",
    patient_display: "Anita Sharma",
  },
  {
    id: "80000000-0000-4000-8000-000000000002",
    user_id: USR_LAB,
    role: "lab_tech",
    resource_type: "lab_orders",
    resource_id: "40000000-0000-4000-8000-000000004401",
    patient_id: PAT2,
    purpose_code: "diagnostics",
    access_channel: "ui",
    emergency_access: false,
    consent_verified: true,
    accessed_at: "2026-07-03T05:55:00.000Z",
    user_display: "S. Nair",
    patient_display: "Rahul Verma",
  },
  {
    id: "80000000-0000-4000-8000-000000000003",
    user_id: USR_ADM,
    role: "admin",
    resource_type: "patients",
    resource_id: null,
    patient_id: PAT1,
    purpose_code: "quality_review",
    access_channel: "export",
    emergency_access: false,
    consent_verified: true,
    accessed_at: "2026-07-12T10:41:00.000Z",
    user_display: "A. Choudhary",
    patient_display: "Anita Sharma",
  },
  {
    id: "80000000-0000-4000-8000-000000000004",
    user_id: USR_DOC,
    role: "doctor",
    resource_type: "visits",
    resource_id: VIS1,
    patient_id: PAT1,
    purpose_code: "emergency_care",
    access_channel: "ui",
    emergency_access: true,
    consent_verified: false,
    accessed_at: "2026-07-14T18:05:00.000Z",
    user_display: "Dr. K. Iyer",
    patient_display: "Anita Sharma",
  },
  {
    id: "80000000-0000-4000-8000-000000000005",
    user_id: USR_RAD,
    role: "radiology_tech",
    resource_type: "radiology_orders",
    resource_id: "40000000-0000-4000-8000-000000007701",
    patient_id: PAT2,
    purpose_code: "diagnostics",
    access_channel: "api",
    emergency_access: false,
    consent_verified: true,
    accessed_at: "2026-07-15T07:12:00.000Z",
    user_display: "M. Rao",
    patient_display: "Rahul Verma",
  },
];

const SEED_FILE_ACCESS: FileAccessLog[] = [
  {
    id: "90000000-0000-4000-8000-000000000001",
    file_id: "a0000000-0000-4000-8000-000000000901",
    user_id: USR_DOC,
    action: "view",
    ip_address: "10.0.2.44",
    accessed_at: "2026-07-11T03:50:00.000Z",
    user_display: "Dr. K. Iyer",
    file_name: "discharge_summary.pdf",
  },
  {
    id: "90000000-0000-4000-8000-000000000002",
    file_id: "a0000000-0000-4000-8000-000000000901",
    user_id: USR_DOC,
    action: "download",
    ip_address: "10.0.2.44",
    accessed_at: "2026-07-11T03:51:12.000Z",
    user_display: "Dr. K. Iyer",
    file_name: "discharge_summary.pdf",
  },
  {
    id: "90000000-0000-4000-8000-000000000003",
    file_id: "a0000000-0000-4000-8000-000000000912",
    user_id: USR_LAB,
    action: "upload",
    ip_address: "10.0.3.12",
    accessed_at: "2026-07-12T05:35:00.000Z",
    user_display: "S. Nair",
    file_name: "cbc_report.pdf",
  },
  {
    id: "90000000-0000-4000-8000-000000000004",
    file_id: "a0000000-0000-4000-8000-000000000920",
    user_id: USR_ADM,
    action: "delete_attempt",
    ip_address: "10.0.0.5",
    accessed_at: "2026-07-14T09:30:00.000Z",
    user_display: "A. Choudhary",
    file_name: "legacy_scan.dcm",
  },
  {
    id: "90000000-0000-4000-8000-000000000005",
    file_id: "a0000000-0000-4000-8000-000000000930",
    user_id: USR_RAD,
    action: "view",
    ip_address: "10.0.5.19",
    accessed_at: "2026-07-16T08:14:00.000Z",
    user_display: "M. Rao",
    file_name: "chest_pa.dcm",
  },
];

const SEED_INTEGRITY: AuditIntegrityCheck[] = [
  {
    id: "b0000000-0000-4000-8000-000000000001",
    facility_id: FACILITY_ID,
    partition_name: "audit_logs_2026_06",
    checked_at: "2026-07-01T02:00:00.000Z",
    rows_checked: 48210,
    chain_valid: true,
    signatures_valid: 48210,
    signatures_invalid: 0,
    first_mismatch_id: null,
    alerted: false,
  },
  {
    id: "b0000000-0000-4000-8000-000000000002",
    facility_id: FACILITY_ID,
    partition_name: "audit_logs_2026_07",
    checked_at: "2026-07-18T02:00:00.000Z",
    rows_checked: 1240,
    chain_valid: false,
    signatures_valid: 1238,
    signatures_invalid: 2,
    first_mismatch_id: "70000000-0000-4000-8000-000000000010",
    alerted: true,
  },
];

const SEED_ARCHIVE: AuditLogArchive[] = [
  {
    id: "c0000000-0000-4000-8000-000000000001",
    facility_id: FACILITY_ID,
    partition_name: "audit_logs_2026_05",
    period_start: "2026-05-01",
    period_end: "2026-05-31",
    row_count: 51002,
    object_storage_bucket: "healthdoc-audit-archive",
    object_storage_key: `${FACILITY_CODE}/2026/05/audit_logs.parquet`,
    archive_file_hash: h("archive-2026-05"),
    archived_at: "2026-06-02T04:00:00.000Z",
    verified_at: "2026-06-02T05:10:00.000Z",
    verification_status: "verified",
  },
];

let auditStore: AuditLog[] = structuredClone(SEED_AUDIT);
let dataAccessStore: DataAccessLog[] = structuredClone(SEED_DATA_ACCESS);
let fileAccessStore: FileAccessLog[] = structuredClone(SEED_FILE_ACCESS);
let integrityStore: AuditIntegrityCheck[] = structuredClone(SEED_INTEGRITY);
let archiveStore: AuditLogArchive[] = structuredClone(SEED_ARCHIVE);

export function getAuditStore(): AuditLog[] {
  return auditStore;
}

export function getAuditDataAccessStore(): DataAccessLog[] {
  return dataAccessStore;
}

export function getFileAccessStore(): FileAccessLog[] {
  return fileAccessStore;
}

export function getIntegrityStore(): AuditIntegrityCheck[] {
  return integrityStore;
}

export function getArchiveStore(): AuditLogArchive[] {
  return archiveStore;
}

/** Test helper — reset between hot reloads if needed */
export function resetAuditMock() {
  auditStore = structuredClone(SEED_AUDIT);
  dataAccessStore = structuredClone(SEED_DATA_ACCESS);
  fileAccessStore = structuredClone(SEED_FILE_ACCESS);
  integrityStore = structuredClone(SEED_INTEGRITY);
  archiveStore = structuredClone(SEED_ARCHIVE);
}
